var control = {
	...
	getDirectionCode: function(angle) {
		var code = 0;
    	if (angle == "left"){
    		code = 65;
    	} else if (angle == "right") {
    ...
    touchInit: function(opt) {
    	control.manager = nipplejs.create(opt);
		control.manager
			.on('dir', function(event, data) {
				if (data.direction && (control.state_up || data.direction.angle != control.angle)) {
					console.log('move ' + data.direction.angle);
					
					control.state_up = false;
					
					if (control.angle == null) {
    					control.angle = data.direction.angle;
					}
					
					if (data.direction.angle != control.angle) {
						console.log('keyup');
    					control.open({
    						type: "keyup",
    						keyCode: control.getDirectionCode(control.angle)
						});
						control.angle = data.direction.angle;
						control.state_up = true;
    				} else {
						console.log('keydown');
						var code = control.getDirectionCode(data.direction.angle);
    					control.open({
    						type: "keydown",
    						keyCode: code
    					});
    		...
    		// 	if (data.direction && !control.working) {
    		// 		console.log('move ' + data.direction.angle);
    		...
    		.on('end', function(event, data) {
				console.log('end ' + control.angle);
				var code = control.getDirectionCode(control.angle);
				control.open({
					type: "keyup",
					keyCode: code
				});
