var game = function() { //添加四步，声明对象，初始化对象init，重写active方法,加载load，更新update
	this.status; //控制主循环更新
	this.ifstop = 0; //是否暂停
...
game.prototype.gameLoad = function(status) { //加载
	...
	switch(status) {
		case 0:
			this.load = [mainscene, newbtn, popup];
			break;
	...
	for(var i = 0; i < game.load.length; i++) { //每次都检查更新一次加载进度
		if(this.load[i].ready) {
			this.loadnum += 1;
		}
	}
	console.log("before，" + this.loadnum);

	/*绘制加载界面*/
	ctx.font = 40 * SCALE + "px nj";
	ctx.fillStyle = "#fff";
	ctx.fillText("加载中..." + parseInt(this.loadnum / this.load.length * 100) + "%", 440 * SCALE, 360 * SCALE);
